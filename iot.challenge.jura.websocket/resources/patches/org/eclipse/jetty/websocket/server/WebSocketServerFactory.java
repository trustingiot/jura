 ...
 */
public class WebSocketServerFactory extends ContainerLifeCycle
	...
	/**
	 * Entry point for
	 * {@link org.eclipse.jetty.websocket.servlet.WebSocketServletFactory.Loader}
	 *
	 * @param context
	 *            the servlet context
	 * @param policy
	...
	private WebSocketServerFactory(ServletContext context, WebSocketPolicy policy, DecoratedObjectFactory objectFactory,
			Executor executor, ByteBufferPool bufferPool) {
	...
	@Override
	public boolean acceptWebSocket(WebSocketCreator creator, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		...
		try {
			...
			HttpConnection connection = (HttpConnection) request
	...
	private WebSocketSession createSession(URI requestURI, EventDriver websocket, LogicalConnection connection) {
		...
		throw new InvalidWebSocketException(
				"Unable to create Session: unrecognized internal EventDriver type: " + websocket.getClass().getName());
	}
	...
	public Object createWebSocket(ServletUpgradeRequest req, ServletUpgradeResponse resp) {
		if (registeredSocketClasses.size() < 1) {
			throw new WebSocketException(
					"No WebSockets have been registered with the factory.  Cannot use default implementation of WebSocketCreator.");
		}

		if (registeredSocketClasses.size() > 1) {
			LOG.warn(
					"You have registered more than 1 websocket object, and are using the default WebSocketCreator! Using first registered websocket.");
		}
	...
	// Using the debugger it's possible to verify that 'context' contains a map with
	// a DecoratedObjectFactory's instance. Maybe the bug is in getAttribute method:
	// http://git.eclipse.org/c/equinox/rt.equinox.bundles.git/tree/bundles/org.eclipse.equinox.http.servlet/src/org/eclipse/equinox/http/servlet/internal/servlet/ServletContextAdaptor.java
	protected void juraPatch() {
		if (this.objectFactory == null) this.objectFactory = new DecoratedObjectFactory();
	}
	...
	public SslContextFactory getSslContextFactory() {
		/*
		 * Not relevant for a Server, as this is defined in the Connector configuration
		 */
		return null;
	}
	 ...
	 * UpgradeConnectionException, to exit HTTP handling and initiate WebSocket
	 * handling of the connection.
	 *
	 * @param http
	 *            the raw http connection
	 * @param request
	 ...
	 */
	private boolean upgrade(HttpConnection http, ServletUpgradeRequest request, ServletUpgradeResponse response,
			EventDriver driver) throws IOException {
		...
		AbstractWebSocketConnection wsConnection = new WebSocketServerConnection(endp, executor, scheduler,
				driver.getPolicy(), bufferPool);
		...
		if (LOG.isDebugEnabled())
			LOG.debug("Websocket upgrade {} {} {} {}", request.getRequestURI(), version,
					response.getAcceptedSubProtocol(), wsConnection);
